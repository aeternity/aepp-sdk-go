...
func createSpendTransaction(senderID, recipientID, payload string, amount, fee, ttl int64, nonce uint64) (rlpRawMsg []byte, err error) {
...
func namePreclaimTx(accountID, commitmentID string, fee, ttl int64, nonce uint64) (rlpRawMsg []byte, err error) {
	// build id for the sender
...
func nameClaimTx(accountID, name string, nameSalt, fee, ttl int64, nonce uint64) (rlpRawMsg []byte, err error) {
	// build id for the sender
	aID, err := buildIDTag(IDTagAccount, accountID)
	if err != nil {
		return
...
func nameUpdateTx(accountID, nameID string, pointers []string, nameTTL, clientTTL, fee, ttl int64, nonce uint64) (rlpRawMsg []byte, err error) {
	// build id for the sender
	aID, err := buildIDTag(IDTagAccount, accountID)
	if err != nil {
		return
