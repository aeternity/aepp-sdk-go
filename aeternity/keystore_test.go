...
func TestWrite(t *testing.T) {
  Config.NewProfile("test")
  Config.ActivateProfile("test")
  Config.Defaults()
  ...
  for _, tt := range tests {
    t.Run(tt.name, func(t *testing.T) {
      encryped, err := KeystoreSeal(tt.args.account, tt.args.password)
      if err != nil {
        t.Errorf("Error %s", err)
        return
      }
      fmt.Printf("%s", encryped)
      decrypted, err := KeystoreOpen(encryped, tt.args.password)
      if err != nil {
        t.Errorf("Error %s", err)
        return
      }
