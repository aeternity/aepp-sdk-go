...
// 			gotTx := aeternity.NewSpendTx(tt.args.senderID, tt.args.recipientID, tt.args.amount, tt.args.fee, tt.args.payload, tt.args.ttl, tt.args.nonce)
...
func TestSpendTx_RLP(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if gotTx != tt.wantTx {
				gotTxRawBytes := aeternity.DecodeRLPMessage(gotTxRaw)
				wantTxRaw, _ := aeternity.Decode(tt.wantTx)
				wantTxRawBytes := aeternity.DecodeRLPMessage(wantTxRaw)
...
func TestOracleRegisterTx_RLP(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if gotTx != tt.wantTx {
				t.Errorf("OracleRegisterTx.RLP() = %v, want %v", gotTx, tt.wantTx)
			}
		})
	}
}
